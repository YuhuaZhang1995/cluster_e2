...
for iter in range(0,200):
	...
	for s in range(0,N):
		...
		for k in range(0,K):
			...
			tmp_p[k]=tmp_p[k]*p
			###Add on May_25###
			p=(alpha_zero[k])/(sum(alpha_zero))
			tmp_p[k]=tmp_p[k]*p
			if int(s_list[s]) in interactions:
				tmp_list=np.array(interactions[int(s_list[s])])
				ct=0
				p=1
				for sct in tmp_list:
					loc=np.where(np.array(s_list)==sct)[0]
					if C_s[loc]==k:
						ct+=1
					p=p*B[k,C_s[loc]]
				#p=p*math.factorial(len(tmp_list))/math.factorial(ct)/math.factorial(len(tmp_list)-ct)
				p=p*math.exp(math.lgamma(len(tmp_list)+1)-math.lgamma(ct+1)-math.lgamma(len(tmp_list)-ct+1))
				tmp_p[k]=tmp_p[k]*p
			
			if int(s_list[s]) in interactions2:
				tmp_list=np.array(interactions2[int(s_list[s])])
				ct=0
				p=1
				for sct in tmp_list:
					loc=np.where(np.array(s_list)==sct)[0]
